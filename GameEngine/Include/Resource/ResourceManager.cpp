...
bool CResourceManager::Init()
{
	...
	m_ParticleManager = DBG_NEW CParticleManager;

	if (!m_ParticleManager->Init())
		return false;

	m_AnimationManager3D = DBG_NEW CAnimationManager3D;
...
void CResourceManager::GetVolume(const std::string& ChannelGroupName, float* Volume)
{
	m_SoundManager->GetVolume(ChannelGroupName, Volume);
}
...
bool CResourceManager::SetVolume(const std::string& ChannelGroupName, float Volume)
{
	return m_SoundManager->SetVolume(ChannelGroupName, Volume);
}
...
bool CResourceManager::SoundPlay(const std::string& Name, class CGameObject* Obj)
{
	return m_SoundManager->SoundPlay(Name, Obj);
}
...
bool CResourceManager::IsSoundPlay(const std::string& Name)
{
	return m_SoundManager->IsSoundPlay(Name);
}
