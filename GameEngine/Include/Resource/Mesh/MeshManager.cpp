...
bool CMeshManager::Init()
{
	...
	CMesh* ParticlePointMesh = DBG_NEW CStaticMesh;
	...
	CreateMesh(Mesh_Type::Static, "Box3DPos", &Box3DPos[0], sizeof(Vector3), 8,
		D3D11_USAGE_DEFAULT, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST,
		&Box3DIndex[0], sizeof(int), 36,
		D3D11_USAGE_DEFAULT, DXGI_FORMAT_R32_UINT);

	int	Box3DLineIndex[24] =
	{
		0, 1, 1, 3, 2, 3, 0, 2, 4, 5, 5, 7, 7, 6, 4, 6, 0, 4, 1, 5, 2, 6, 3, 7
	};

	// 라인을 그릴거라서 위상구조를 LineList로한다
	CreateMesh(Mesh_Type::Static, "Box3DLinePos",
		&Box3DPos[0], sizeof(Vector3), 8,
		D3D11_USAGE_DEFAULT, D3D11_PRIMITIVE_TOPOLOGY_LINELIST,
		&Box3DLineIndex[0], sizeof(int), 24,
		D3D11_USAGE_DEFAULT, DXGI_FORMAT_R32_UINT);
	...
	D3D11_USAGE IdxUsage, DXGI_FORMAT Fmt, CScene* Scene)
{
	...
	case Mesh_Type::Sprite:
		Mesh = DBG_NEW CSpriteMesh;
		break;
	case Mesh_Type::Static:
		Mesh = DBG_NEW CStaticMesh;
		break;
	case Mesh_Type::Animation:
		Mesh = DBG_NEW CAnimationMesh;
		break;
	}
