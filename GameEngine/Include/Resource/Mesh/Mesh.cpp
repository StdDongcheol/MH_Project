	...
	void* IdxData, int IdxSize, int IdxCount, D3D11_USAGE IdxUsage, DXGI_FORMAT Fmt)
{
	...
	Container->VB.Data = DBG_NEW char[Size * Count];
	memcpy(Container->VB.Data, VtxData, Size * Count);

	MeshSlot* Slot = DBG_NEW MeshSlot;
	...
	if (IdxData != nullptr)
	{
		...
		Container->vecIB[0].Data = DBG_NEW char[IdxSize * IdxCount];
...
bool CMesh::ConvertFBX(CFBXLoader* Loader, const char* FullPath)
{
	...
	for (; iter != iterEnd; ++iter, ++ContainerIndex)
	{
		...
		Container->VB.Data = DBG_NEW char[Container->VB.Size * Container->VB.Count];
		...
		for (size_t i = 0; i < IdxCount; ++i)
		{
			...
			Container->vecIB[i].Data = DBG_NEW char[Container->vecIB[i].Size * Container->vecIB[i].Count];
	...
	for (; iterM != iterMEnd; ++iterM, ++ContainerIndex)
	{
		...
		for (size_t i = 0; i < Size; ++i)
		{
			...
			CMaterial* Material = DBG_NEW CMaterial;
...
bool CMesh::LoadMesh(FILE* File)
{
	...
	for (int i = 0; i < ContainerCount; ++i)
	{
		...
		for (int j = 0; j < IdxCount; ++j)
		{
			...
			Container->vecIB[j].Data = DBG_NEW char[Container->vecIB[j].Size * Container->vecIB[j].Count];
...
void CMesh::UpdateVertex(void* Data, UINT Size)
{
	CDevice::GetInst()->GetContext()->UpdateSubresource(m_vecMeshSlot[0]->VB->Buffer, 0, nullptr, Data, (UINT)(m_vecMeshSlot[0]->VB->Size), Size);
}
