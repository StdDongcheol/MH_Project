...
bool CShaderManager::Init()
{
	...
	if (!CreateShader<CEffectShader>("EffectShader"))
		return false;


	if (!CreateShader<CRimLightShader>("RimLightShader"))
		return false;

	if (!CreateShader<CPickingComputeShader>("PickingComputeShader"))
		return false;
	...
	if (!CreateShader<CMotionBlurShader>("MotionBlurShader"))
		return false;
	...
	CreateConstantBuffer("RimLightCBuffer", sizeof(RimLightCBuffer), 13,
		(int)Buffer_Shader_Type::Graphic);

	CreateConstantBuffer("PickingCBuffer", sizeof(PickingCBuffer), 13,
		(int)Buffer_Shader_Type::Compute);
