...
public:
	...
	void SetParticleRot(const Vector3& Rot)
	{
		m_BufferData.ParticleRot = Rot;
	}

	void SetComponentAngle(const Vector3& Angle)
	{
		m_BufferData.ComponentAngle = Angle;
	}

	void SetPlayerInvRot(const Matrix& matRot)
	{
		m_BufferData.PlayerInvRot = matRot;
	}

	void SetCameraAxisX(const Vector3& AxisX)
	{
		m_BufferData.CameraAxisX = AxisX;
	}

	void SetCameraAxisY(const Vector3& AxisY)
	{
		m_BufferData.CameraAxisY = AxisY;
	}

	void SetParticleAlive(int Alive)
	{
		m_BufferData.IsAlive = Alive ? 1 : 0;
	}
};
