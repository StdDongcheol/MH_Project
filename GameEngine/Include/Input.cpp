...
void CInput::PlayCallback(const std::string& Name, Key_State State)
{
	KeyInfo* Info = FindKeyInfo(Name);
	...
	Info->Callback[State](CEngine::GetInst()->GetDeltaTime());
}
...
void CInput::Update(float DeltaTime)
{
	...
	{
		if (!CEngine::GetInst()->IsFocus() && !CIMGUIManager::GetInst()->IsFocus())
			return;
	}

	else
	{
		if (!CEngine::GetInst()->IsFocus())
			return;
	}
...
bool CInput::IsPush(const std::string& Name)
{
	KeyInfo* Info = FindKeyInfo(Name);
	...
	unsigned char Key = Info->State.Key;

	return m_vecKeyState[Key].State[KeyState_Push];
}

bool CInput::IsUp(const std::string& Name)
{
	KeyInfo* Info = FindKeyInfo(Name);
	...
	return m_vecKeyState[Key].State[KeyState_Up];
}

bool CInput::IsDown(const std::string& Name)
{
	KeyInfo* Info = FindKeyInfo(Name);
	...
	return m_vecKeyState[Key].State[KeyState_Down];
}