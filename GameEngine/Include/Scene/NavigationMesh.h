...
private:
	string		m_Name;
	vector<NavigationCell*>	m_vecCell;	// ³×ºñ¸¦ À§ÇÑ »ï°¢Çü ¼¿
...
public:
	bool Init();
	void ClearCell();
	void AddCell(const Vector3 Pos[3], int TextureType[3]);
	...
	bool GetMoveEnable(const Vector3& Pos);

public:
	float GetY(const Vector3& Pos);
	int CheckCellMat(const Vector3& Pos);
