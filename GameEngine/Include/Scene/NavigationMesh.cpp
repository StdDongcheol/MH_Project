...
CNavigationMesh::CNavigationMesh()
{
	...
	m_SectionX = 1;
	m_SectionZ = 1;
	m_Section = DBG_NEW NavSection[m_SectionX * m_SectionZ];
...
void CNavigationMesh::AddCell(const Vector3 Pos[3], int TextureType[3])
{
	NavigationCell* Cell = DBG_NEW NavigationCell;
	...
	// 두개 이상의 버텍스가 같은 재질일 경우에 셀에 해당 텍스쳐 타입을 지정한다.
	if ((TextureType[0] == TextureType[1]) && (TextureType[0] == TextureType[2]))
	{
		Cell->TextureType = TextureType[0];
	}

	else if((TextureType[0] == TextureType[1]) && (TextureType[1] == TextureType[2]))
	{
		Cell->TextureType = TextureType[1];
	}

	else if ((TextureType[1] == TextureType[2]) && (TextureType[0] == TextureType[2]))
	{
		Cell->TextureType = TextureType[2];
	}
	...
	const Vector3& End)
{
	...
	if (!m_UseCellList)
	{
		m_UseCellList = DBG_NEW NavigationCell * [m_vecCell.size()];
		m_OpenList = DBG_NEW NavigationCell * [m_vecCell.size()];
...
bool CNavigationMesh::GetMoveEnable(const Vector3& Pos)
{
	NavigationCell* PlayerPosCell = FindCell(Pos);

	if (!PlayerPosCell)
		return false;

	if (PlayerPosCell->Enable)
		return true;
...
int CNavigationMesh::CheckCellMat(const Vector3& Pos)
{
	...
	return m_vecCell[CellIndex]->TextureType;
}
...
void CNavigationMesh::CreateSection()
{
	...
	for (int i = 0; i < m_SectionZ; ++i)
	{
		for (int j = 0; j < m_SectionX; ++j)
		{
			...
			m_Section[idx].Min = m_Min + m_SectionSize * Vector3((float)j, 0, (float)i);
			m_Section[idx].Max = m_Min + m_SectionSize * Vector3((float)(j + 1), (float)1, (float)(i + 1));
		}
	}
...
void CNavigationMesh::Save(FILE* File)
{
	...
	for (size_t i = 0; i < Size; ++i)
	{
		...
		fwrite(&m_vecCell[i]->TextureType, sizeof(int), 1, File);
...
void CNavigationMesh::Load(FILE* File)
{
	...
	for (size_t i = 0; i < Size; ++i)
	{
		...
		fread(&Cell->TextureType, sizeof(int), 1, File);
