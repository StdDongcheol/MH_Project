...
CNavigationMesh* CNavigation3DManager::AddNavData(CLandScape* NavData)
{
	...
	NavMesh = DBG_NEW CNavigationMesh;
...
int CNavigation3DManager::CheckCellMat(const Vector3& Pos)
{
	CNavigationMesh* pNavMesh = FindNavMesh(Pos);
	...
	return pNavMesh->CheckCellMat(Pos);
}
...
bool CNavigation3DManager::GetMoveEnable(const Vector3& Pos)
{
	CNavigationMesh* pNavMesh = FindNavMesh(Pos);
	...
	return pNavMesh->GetMoveEnable(Pos);
}
...
void CNavigation3DManager::Save(FILE* File, CLandScape* NavData)
{
	CNavigationMesh* NavMesh = FindNavMesh(NavData->GetName());
...
void CNavigation3DManager::Load(FILE* File, CLandScape* NavData)
{
	size_t	Size = m_vecNavigationThread.size();
	...
	CNavigationMesh* NavMesh = DBG_NEW CNavigationMesh;
	NavMesh->Load(File);

	m_mapNavMesh.insert(std::make_pair(NavMesh->GetName(), NavMesh));
}
