...
void CEngine::ShowUICursor(bool bEnable)
{
	for (int i = 0; i < (int)Mouse_State::Max; ++i)
	{
		if (m_MouseWidget[i] == nullptr)
		{
			continue;
		}

		m_MouseWidget[i]->SetVisibility(bEnable);
	}
}
	...
	unsigned int Width, unsigned int Height, bool WindowMode)
{
	...
	// 난수 전용 구조화버퍼 생성
	//m_RandomBuffer = DBG_NEW CStructuredBuffer;
	...
	if (!CHDR::GetInst()->Init(m_RS.Width, m_RS.Height))
		return false;
...
bool CEngine::Update(float DeltaTime)
{
	...
	{
		// 마우스 위치를 얻어온다.
		Vector2 Pos = CInput::GetInst()->GetMouseUIPos();
		...
		if (Mouse_State::Normal == m_MouseState)
		{
			Pos.y -= m_MouseWidget[(int)m_MouseState]->GetWindowSize().y -8.f;
			Pos.x -= m_MouseWidget[(int)m_MouseState]->GetWindowSize().x / 3.f;
		}
		else
...
{
	// 최대화, 창 크기 조절 비활성화
	m_hWnd = CreateWindowW(Name, Name, WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX & ~WS_THICKFRAME, 0, 0, m_RS.Width, m_RS.Height, nullptr, nullptr, m_hInst, nullptr);
	...
	int width = (GetSystemMetrics(SM_CXSCREEN) / 2) - (m_RS.Width / 2);

	int height = (GetSystemMetrics(SM_CYSCREEN) / 2) - (m_RS.Height / 2);

	// 위에서 얻어온 Rect를 이용해서 윈도우 크기를 지정한다.
	SetWindowPos(m_hWnd, HWND_TOPMOST, width, height, rc.right - rc.left, rc.bottom - rc.top, SWP_NOZORDER);
