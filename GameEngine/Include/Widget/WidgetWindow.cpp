...
CWidgetWindow::CWidgetWindow()	:
	...
	m_Start(false),
	m_Visibility(true),
	m_EnableFadeIn(false),
	m_EnableFadeOut(false),
	m_FadeDestroy(false),
	m_FadeSpeed(3.f)
{
}
...
void CWidgetWindow::FadeEnable(bool bEnable)
{
	if (bEnable)
	{
		CRef::Enable(true);
	...
	{
		m_EnableFadeOut = true;
	}
}

void CWidgetWindow::FadeDestroy()
{
	m_EnableFadeOut = true;
	m_FadeDestroy = true;
}
...
void CWidgetWindow::Render()
{
	...
	// false일 경우, WidgetList Render만 하지않음.
	if (!m_Visibility)
		return;

	if (m_EnableFadeIn)
	{
		float WidgetOpacity = 1.f;

		auto	iter1 = m_WidgetList.begin();
		auto	iter1End = m_WidgetList.end();

		for (; iter1 != iter1End; ++iter1)
		{
			(*iter1)->SetFadeIn(true);
			(*iter1)->SetFadeSpeed(m_FadeSpeed);

			float Opacity = (*iter1)->GetOpacity();

			WidgetOpacity = Opacity < WidgetOpacity ? Opacity : WidgetOpacity;
	...
	else if (m_EnableFadeOut)
	{
		...
		for (; iter1 != iter1End; ++iter1)
		{
			(*iter1)->SetFadeOut(true);
			(*iter1)->SetFadeSpeed(m_FadeSpeed);

			// 반복(Flicker) 위젯일경우, 제외.
			if ((*iter1)->m_FadeRepeat)
			{
				continue;
			}
