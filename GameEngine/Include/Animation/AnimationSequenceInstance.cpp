	...
	float PlayTime, float PlayScale)
{
	...
	// 각각의 데이터 정보를 넣어준다.
	Sequence->m_PlayTime = PlayTime;
	Sequence->m_PlayScale = PlayScale;
	Sequence->m_FrameTime = PlayTime / Sequence->GetKeyFrameCount();
...
bool CAnimationSequenceInstance::CheckChangeAnimation(const std::string& Name)
{
	if (!m_ChangeAnimation)
		return false;

	return m_ChangeAnimation->m_Name == Name;
}


void CAnimationSequenceInstance::Start()
{
	if (!m_AnimationUpdateCBuffer && !m_OutputBuffer && !m_BoneBuffer)
	{
		if (m_Scene)
		{
...
void CAnimationSequenceInstance::Update(float DeltaTime)
{
	...
	else
	{
		...
		// 이전프레임과 얼마나 차이나는지 계산
		int CurrFrameSize = FrameIndex - m_PrevFrameIndex;
		
		// m_PrevFrameIndex가 이전 애니메이션의 프레임 들고있었을 경우를 대비.
		
		// 이 조건에 걸렸을경우, 이미 다음 애니메이션으로 진행해버렸으므로 
		// 이전 애니메이션을 불러와 Notify까지 재생하기 어려울거 같음. 
		// 그래서 차선으로 현재 애니메이션의 Notify까지만이라도 재생하도록 구현.
		if (CurrFrameSize == FrameIndex)
		{
			m_PrevFrameIndex = 0;

			// 현재 재생중인 프레임이 CurrFrameSize가 된다.
			// 현재 재생중인 프레임의 수만큼 다시 재생시켜주어야함.
			CurrFrameSize = FrameIndex;
		}

		// 현재프레임과 이전프레임이 2 이상 차이날경우
		// Notify 프레임 누락검사 시작.
		else if (CurrFrameSize >= 1)
		{
			// 기본 1.
			int i = 1;
			
			// 이전 애니메이션의 프레임을 들고있었을경우 
			// 현재 애니메이션 Frame의 0부터
			// 재생해야하므로 i를 0으로.
			if (m_PrevFrameIndex == 0)
			{
				i = 0;
			}

			for (; i < CurrFrameSize; ++i)
			{
				size_t	Size = m_CurrentAnimation->m_vecNotify.size();

				for (size_t j = 0; j < Size; ++j)
				{
					if (!m_CurrentAnimation->m_vecNotify[j]->Call &&
						m_CurrentAnimation->m_vecNotify[j]->Frame == m_PrevFrameIndex + i)
					{
						m_CurrentAnimation->m_vecNotify[j]->Call = true;
						m_CurrentAnimation->m_vecNotify[j]->Function();
					}
				}
			}
		}
