...
bool CRenderManager::Init()
{
	...
	CDevice::GetInst()->GetDevice()->QueryInterface<ID3D10Multithread>(&MultiThread);
	...
	if (!CResourceManager::GetInst()->CreateTarget("GBuffer6",
		RS.Width, RS.Height, DXGI_FORMAT_R32G32B32A32_FLOAT))
		return false;
	...
	GBufferTarget->SetPos(Vector3(100.f, 600.f, 0.f));
	GBufferTarget->SetScale(Vector3(100.f, 100.f, 1.f));
	GBufferTarget->SetDebugRender(true);
	m_vecGBuffer.push_back(GBufferTarget);
	...
	if (!CResourceManager::GetInst()->CreateTarget("VelocityMap",
		RS.Width, RS.Height, DXGI_FORMAT_R32G32B32A32_FLOAT))
		return false;

	m_ShadowMapTarget = (CRenderTarget*)CResourceManager::GetInst()->FindTexture("ShadowMap");
	m_ShadowMapTarget->SetPos(Vector3(500.f, 0.f, 0.f));
	m_ShadowMapTarget->SetScale(Vector3(200.f, 200.f, 1.f));
	m_ShadowMapTarget->SetDebugRender(true);

	// Motion Blur 처리용 VelocityMap Target
	m_VelocityMapTarget = (CRenderTarget*)CResourceManager::GetInst()->FindTexture("VelocityMap");
	m_VelocityMapTarget->SetPos(Vector3(800.f, 0.f, 0.f));
	m_VelocityMapTarget->SetScale(Vector3(200.f, 200.f, 1.f));
	m_VelocityMapTarget->SetDebugRender(true);
	...
	m_EffectShader = CResourceManager::GetInst()->FindShader("EffectShader");
	m_VelocityCreateShader = CResourceManager::GetInst()->FindShader("MotionBlurShader");
...
void CRenderManager::RenderVelocityMap()
{
	ID3D11RenderTargetView*	Target = nullptr;
	ID3D11RenderTargetView*	PrevTarget = nullptr;
	ID3D11DepthStencilView* PrevDepthTarget = nullptr;

	m_vecGBuffer[6]->ClearTarget();
	Target = m_vecGBuffer[6]->GetTargetView();
	
	// 현재 지정되어 있는 렌더타겟과 깊이타겟을 얻어온다.
	CDevice::GetInst()->GetContext()->OMGetRenderTargets(1, &PrevTarget, &PrevDepthTarget);

	CDevice::GetInst()->GetContext()->OMSetRenderTargets(1, &Target, PrevDepthTarget);
	...
	for (size_t i = 0; i <= 1; ++i)
	{
		...
		for (; iter != iterEnd; ++iter)
		{
			(*iter)->RenderVelocityMap();
		}
	}

	CDevice::GetInst()->GetContext()->OMSetRenderTargets(1, &PrevTarget, PrevDepthTarget);
...
void CRenderManager::RenderFinalScreen()
{
	...
	//hdr 적용 부분
	if (CHDR::GetInst()->GetHDREnable())
	{
		CHDR::GetInst()->PostProcessing(CDevice::GetInst()->GetContext(), 
			FinalScreenTarget->GetResource(0), FinalScreenTarget->GetTargetView());
	}
	...
	CHDR::GetInst()->CleanUp(CDevice::GetInst()->GetContext());
}

void CRenderManager::RenderEffect()
{
	m_AlphaBlend->SetState();
	//m_vecGBuffer[0]->SetTargetShader(30);

	//m_vecGBuffer[2]->SetTargetShader(31);

	m_EffectShader->SetShader();
	...
	//m_vecGBuffer[0]->ResetTargetShader(30);

	//m_vecGBuffer[2]->ResetTargetShader(31);
