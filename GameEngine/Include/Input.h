...
public:
	void PlayCallback(const std::string& Name, Key_State State);
...
public:
	bool IsPush(const std::string& Name);
...
public:
	...
	void SetKeyCallback(const std::string& Name, Key_State State, T* Obj, void(T::* Func)())
	{
		...
		Info->Callback[State] = std::bind(Func, Obj);
	}
