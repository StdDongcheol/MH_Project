...
namespace ImGui
{
	...
	IMGUI_API bool          IsKeyPressed(int user_key_index, bool repeat = true, float repeatdelay = 0.f, float repeatrate = 0.f);               // was key pressed (went from !Down to Down)? if repeat=true, uses io.KeyRepeatDelay / KeyRepeatRate
	...
	IMGUI_API bool          IsMouseClicked(ImGuiMouseButton button, bool repeat = false, float repeatdelay = 0.f, float repeatrate = 0.f);       // did mouse button clicked? (went from !Down to Down). Same as GetMouseClickedCount() == 1.
...
enum ImGuiKey_
{
	// 만약 추가할 경우 imgui_impl_win32.cpp의 ImGui_ImplWin32_Init 함수 내에서도 적용시켜줘야 함.
	ImGuiKey_Tab,
	ImGuiKey_LeftArrow,
	ImGuiKey_RightArrow,
	ImGuiKey_UpArrow,
