...
void CCameraComponent::Start()
{
	...
	// m_TargetMode이면 최초한번만 해당 ViewMat으로 카메라 설정.
	if (m_TargetMode)
	{
		for (int i = 0; i < AXIS_MAX; ++i)
		{
	...
	*/
	...
	if (m_CamShake)
	{
		...
		if (m_LerpStrength)
		{
			...
			// DeltaTime에 누적되는 m_ShakeStrength는 쉐이킹 범위가 점점 넒어짐.
			// 0.1f만큼 DeltaTime을 지연.
			m_ShakeStrength += 0.1f * DeltaTime;

			// DeltaTime에 누적되는 m_ShakeLengthAcc 값 만큼의 주기를 반복.
			Radian = cosf(DegreeToRadian(m_ShakeTimeAcc * m_ShakeLengthAcc));
		}

		else
		{			
			// 고정된 m_ShakeLength 값 만큼의 주기를 반복.
