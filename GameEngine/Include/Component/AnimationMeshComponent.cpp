...
Vector3 CAnimationMeshComponent::GetSocketPos(const std::string& SocketName)
{
	if (!m_Skeleton->GetSocket(SocketName))
	{
		return Vector3().Zero;
	}

	return m_Skeleton->GetSocket(SocketName)->GetSocketPos();
}
...
bool CAnimationMeshComponent::DeleteChild(CSceneComponent* Child)
{
	if (!CSceneComponent::DeleteChild(Child))
		return false;
...
bool CAnimationMeshComponent::DeleteChild(CGameObject* Child)
{
	if (!CSceneComponent::DeleteChild(Child))
		return false;
...
void CAnimationMeshComponent::RenderRimLight()
{
}
...
void CAnimationMeshComponent::RenderShadowMap()
{
	...
	for (size_t i = 0; i < Size; ++i)
	{
		m_vecMaterialSlot[i]->RenderShadow();

		m_Mesh->RenderSetBuffer((int)i);
...
void CAnimationMeshComponent::RenderVelocityMap()
{
	CSceneComponent::RenderVelocityMap();
