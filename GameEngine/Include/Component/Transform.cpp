...
Vector3 CTransform::GetSocketPos() const
{
	return m_Socket->GetSocketPos();
}
...
void CTransform::InheritWorldScale(bool Current)
{
	...
	if (m_Owner->CheckType<CColliderBox3D>())
	{
		// 메쉬가 없는 경우
		if (!m_Parent)
			return;

		CColliderBox3D* m_Box3D = (CColliderBox3D*)m_Owner;
		Vector3 PrevScale = m_Parent->m_WorldScalePrev;
		Vector3 Length = m_Box3D->GetInfo().Length;

		Length.x = PrevScale.x != 0.f ? Length.x * (m_Parent->m_WorldScale.x / PrevScale.x) : 0.f;
		Length.y = PrevScale.y != 0.f ? Length.y * (m_Parent->m_WorldScale.y / PrevScale.y) : 0.f;
		Length.z = PrevScale.z != 0.f ? Length.z * (m_Parent->m_WorldScale.z / PrevScale.z) : 0.f;

		m_Box3D->Set3DExtent(Length.x, Length.y, Length.z);
	}
...
void CTransform::SetRotationAxis(const Matrix& matView)
{
	m_UpdateRotAxis = true;
...
void CTransform::SetTransform()
{
	...
	m_CBuffer->SetPrevWorldMatrix(m_matWorld);
	m_CBuffer->SetPrevViewMatrix(Camera->GetProjMatrix());
}
