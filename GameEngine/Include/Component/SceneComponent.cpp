...
void CSceneComponent::AddChild(CSceneComponent* Child, const std::string& SocketName)
{
	...
	Child->m_Transform->m_WorldPos = m_Transform->m_WorldPos;
...
bool CSceneComponent::DeleteChild(CGameObject* Child)
{
	size_t	Size = m_vecChild.size();

	for (size_t i = 0; i < Size; ++i)
	{
		if (m_vecChild[i] == Child->GetRootComponent())
		{
			auto	iter = m_vecChild.begin() + i;
...
{
	if (m_Render && !m_Culling && !m_Instancing && !m_Invisible)
...
void CSceneComponent::RenderRimLight()
{
	m_Transform->SetTransform();
}
...
void CSceneComponent::RenderVelocityMap()
{
	m_Transform->SetTransform();
}
