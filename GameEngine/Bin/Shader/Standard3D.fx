...
Vertex3DOutput Standard3DVS(Vertex3D input)
{
	...
	output.WorldNormal = normalize(mul(float4(Info.Normal, 0.f), g_matWorld).xyz);
...
PSOutput_GBuffer Standard3DPS(Vertex3DOutput input)
{
	...
	//output.GBuffer6.rgb = BaseTextureColor.rgb;
	//output.GBuffer6.a = BaseTextureColor.a * g_MtrlOpacity;
...
//	int	NumSample = 10;
//
//	float4 Velocity = tex2D(g_PointSmp, input.UV);
//
//	Velocity.xy /= (float)NumSample;
//
	return output;
}
