...
Texture2DMS<float4> g_VelocityMapTex : register(t23);
...
LightResult ComputeLight(float3 Pos, float3 Normal, float4 MaterialColor)
{
    ...
    result.Amb = g_LightColor.xyz * 0.6f * MtrlAmb * Attn;
...
PSOutput_Single LightBlendRenderPS(VS_OUTPUT_LIGHTACC input)
{
   ...
   /* int NumBlurSample = 5;
    ...
    float4 Velocity = g_VelocityMapTex.Load(TargetPos, 0);
    ...
    {
        BColor = g_LightBlendTex.Load((float2)TargetPos + Velocity.xy * (float)i, 0);
    
        if (Velocity.a < BColor.a + 0.04f)
        {
            iCnt++;
            LightBlendColor += BColor;
        }
    }
    LightBlendColor /= (float)iCnt;*/
    ...
    //float tv = 1.f / g_Resolution.y;
    //
    //for (int i = -6; i < 6; ++i)
    //{
    //    uv = t + float2(tu * i, 0);
    //    OutU += Weight[6 + i] * g_LightBlendTex.Load(TargetPos, 0);
    //}
    //
    //OutU /= Total;
    //
