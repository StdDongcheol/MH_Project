...
void ParticleUpdate(uint3 ThreadID : SV_DispatchThreadID)
{
	...
	g_ParticleShare[0].ComponentAngle = g_ParticleComponentAngle;
	g_ParticleShare[0].PlayerInvRot = g_PlayerInvRot;
	g_ParticleShare[0].CameraAxisX = g_CameraAxisX;
	g_ParticleShare[0].CameraAxisY = g_CameraAxisY;

	g_ParticleArray[ThreadID.x].ParticleIsAlive = g_ParticleIsAlive;
	...
	inout TriangleStream<GeometryParticleOutput> output)
{
	...
	if (g_ParticleArraySRV[InstanceID.x].ParticleIsAlive == 0)
		return;
	...
	float3 HalfLT = g_ParticleLocalPos[0] * Scale;
	float3 HalfRT = g_ParticleLocalPos[1] * Scale;
	float3 HalfLB = g_ParticleLocalPos[2] * Scale;
	float3 HalfRB = g_ParticleLocalPos[3] * Scale;
	float3 HalfWidth = 0.5f * Scale.x;
	float3 HalfHeight = 0.5f * Scale.y;

	//sfloat3x3 matRot = ComputeRotationMatrix(g_ParticleShareSRV[0].ComponentAngle);


	Pos[0] = float3(g_ParticleArraySRV[InstanceID].WorldPos + HalfLT.x * g_ParticleShareSRV[0].CameraAxisX + HalfLT.y * g_ParticleShareSRV[0].CameraAxisY);
	Pos[1] = float3(g_ParticleArraySRV[InstanceID].WorldPos + HalfRT.x * g_ParticleShareSRV[0].CameraAxisX + HalfRT.y * g_ParticleShareSRV[0].CameraAxisY);
	Pos[2] = float3(g_ParticleArraySRV[InstanceID].WorldPos + HalfLB.x * g_ParticleShareSRV[0].CameraAxisX + HalfLB.y * g_ParticleShareSRV[0].CameraAxisY);
	Pos[3] = float3(g_ParticleArraySRV[InstanceID].WorldPos + HalfRB.x * g_ParticleShareSRV[0].CameraAxisX + HalfRB.y * g_ParticleShareSRV[0].CameraAxisY);
	//float4 ConvertPos[4];
	//matrix matWVP = mul(g_ParticleShareSRV[0].PlayerInvRot, g_matVP);
	// 4개의 최종 정점정보를 만들어준다.
	for (int i = 0; i < 4; ++i)
	{
		//ConvertPos[i] = mul(float4(Pos[i], 1.f), g_ParticleShareSRV[0].PlayerInvRot);
		OutputArray[i].ProjPos = mul(float4(Pos[i], 1.f), g_matVP);
